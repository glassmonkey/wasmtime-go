...
// result, flags, map) so each [ComponentItem] kind can be downcast to its
// dedicated type. The slice currently exposed here intentionally only
// surfaces the kinds reachable without further sub-types.
// TODO: composite WIT value types (list, record, tuple, variant, enum,
// option, result, flags, map) and resource types — value marshaling
// currently covers primitive scalar and string values.
